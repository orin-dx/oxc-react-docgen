...
- Every one of those gaps degrades to a `Diagnostic` plus an `Opaque` prop type, never a silent wrong answer (`crates/core/CLAUDE.md` non-negotiable #6) — the speed trade-off costs completeness, never correctness.
