/**
 * react-day-picker — `DayPickerProps` (simplified fixture)
 *
 * Adapted from gpbl/react-day-picker (MIT license), the real source file:
 * https://github.com/gpbl/react-day-picker/blob/main/packages/react-day-picker/src/types/props.ts
 * (fetched at ~671 lines, tag/commit on the `main` branch, 2026-07)
 *
 * This is THE fixture for stress-testing a large discriminated union: unlike
 * fixtures/rdt-compat/discriminated-union.tsx (a hand-authored 2-branch
 * union), `DayPickerProps` is real production code with a *double*
 * discriminant — `mode` ("single" | "multiple" | "range") crossed with
 * `required` (true | false/undefined) — producing 7 union branches:
 *   PropsSingle, PropsSingleRequired, PropsMulti, PropsMultiRequired,
 *   PropsRange, PropsRangeRequired, and the bare
 *   `{ mode?: undefined; required?: undefined }` no-selection-mode case.
 *
 * `DayPickerProps` itself and all 7 branch interfaces below (including their
 * JSDoc) are UNCHANGED from upstream — that union shape is exactly what this
 * fixture exists to exercise, so it is not flattened or simplified.
 *
 * `PropsBase` (~50 fields covering navigation, localization, styling, and
 * day-level event handlers) is also kept verbatim since it's real production
 * surface and cheap to keep as-is. What's NOT kept verbatim: the auxiliary
 * types PropsBase references (`Modifiers`, `Formatters`, `Labels`,
 * `ClassNames`, `Styles`, `CustomComponents`, `Numerals`, `DayPickerLocale`)
 * are collapsed to minimal stand-ins in ./shared.ts, and the `dateLib` field
 * (originally `Partial<typeof DateLib.prototype>`) is simplified to drop the
 * dependency on react-day-picker's internal `DateLib` class — none of that
 * affects discriminated-union detection.
 */
...
export interface PropsBase {
   ...
   * @experimental
   */
  // Simplified from `Partial<typeof DateLib.prototype>` — the real DateLib
  // class isn't part of this fixture.
  dateLib?: Record<string, unknown> | undefined;
}
