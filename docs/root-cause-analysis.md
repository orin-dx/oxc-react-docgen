...
| --- | --- | --- |
| Generic alias `.zip()` silently truncates unsubstituted trailing type params | `resolver/substitute.rs:33-43` | Silent *truncation* of a substitution map, not a give-up-and-return-nothing path — closer to a P0 correctness bug than the give-up cluster |
...
| Zero-member `LiteralUnion` serializes as an empty RDT enum | `cli/commands/extract.rs` (`rdt_type_json`) | Same tier as above — low-priority, cosmetic. Confirmed reachable from several live construction paths (`known.rs`, `resolver/primitives.rs`, `resolver/alias.rs`, `resolver/mod.rs`, `resolver/template.rs`), not purely theoretical, but severity assessment holds — cosmetic, not correctness-affecting. |
